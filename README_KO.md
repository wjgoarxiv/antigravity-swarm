...
| 단일 명령 중심 | planner, executor, loop, reviewer, cleanup, goal, diagnostics guidance |
| 정적인 문서 | hook alias와 보이는 workflow handoff |
| 최소 prompt routing | skills, hooks, subagents, LSP diagnostics guidance, HUD |
...
ASW는 alias 감지, 진단 가이드 handoff, 백그라운드 작업 continuation을 위한 hook support를 설치합니다.

### LSP Diagnostics

ASW는 변경 파일의 실제 언어 검사를 유도하는 진단 가이드 hook과 스킬 지시를 포함합니다. 이 hook은 안전한 no-op이며 language server를 직접 실행하지 않습니다. 진단 통과를 주장하기 전에는 `asw-lsp`가 지시하는 프로젝트 고유 검사를 직접 실행하세요.
